#!/usr/bin/env python3
"""
Polymarket Tech Tracker - Culture (Pop-Culture) Long-Term Updater
每天00:00运行：抓取高概率pop-culture事件（70%-99%）并更新Long-Term数据
"""
...
CULTURE_DIR = BASE_DIR / "docs" / "data" / "culture"
LONG_TERM_DIR = CULTURE_DIR / "long_term"

GAMMA_API = "https://gamma-api.polymarket.com/events"

def fetch_high_prob_events(max_retries=3):
    """用curl抓取高概率pop-culture事件（70%-99%）"""
    today = datetime.now().strftime('%Y-%m-%d')
    url = f"{GAMMA_API}?tag_slug=pop-culture&end_date_min={today}&volume_min=10000&limit=1000"
...
def main():
    """主函数"""
    print(f"\n{'='*60}")
    print(f"🎭 Culture (Pop-Culture) Long-Term Update - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"{'='*60}\n")
    
    print("1️⃣  Fetching high-probability pop-culture events (70%-99%)...")
    ...
    print(f"{'='*60}")
    print(f"✅ Culture Long-Term update complete!")
    print(f"{'='*60}\n")
